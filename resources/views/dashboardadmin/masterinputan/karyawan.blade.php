                                    ...
                                    <tbody>
                                        @foreach ($karyawan as $d)
                                            <tr class="odd">
                                                ...
                                                <td>
                                                    @php
                                                        $path = !empty($d->foto) ? Storage::url('uploads/karyawan/img/' . $d->foto) : '';
                                                        $defaultAvatar = $d->jenkel === 'pria' ? 'avatar.jpg' : 'avatarwanita.jpg';
                                                    @endphp
                                                    <img src="{{ !empty($path) ? url($path) : asset('assets/img/' . $defaultAvatar) }}"
                                                        {{ $d->jenkel === 'pria' ? '' : 'img-fluid' }}" style="height:30px">
                                                </td>
                                                <td>{{ $d->nama }}</td>
                                                <td>{{ $d->pendidikan ?? '-' }}</td>
                                                <td>{{ $d->tinggi ?? '-' }} cm</td>
